...
def print_coefs_table(coefs, x, y):
    ...
    print("i" + "\t\t" + "x" + "\t\t\t" + "y" + "\t\t\t" + "h" + "\t\t\t" + "a" + "\t\t\t" + "c" + "\t\t\t" + "b" + "\t\t\t" + "d")

    for i in range(len(c)):
        xt = "{:^10s}".format("None") if i >= len(x) else "{:^10.3f}".format(x[i])
        yt = "{:^10s}".format("None") if i >= len(y) else "{:^10.3f}".format(y[i])
        ht = "{:^10s}".format("None") if i >= len(h) or h[i] == None else "{:^10.3f}".format(h[i])
        at = "{:^10s}".format("None") if i >= len(a) or a[i] == None else "{:^10.3f}".format(a[i])
        ct = "{:^10s}".format("None") if c[i] == None else "{:^10.3f}".format(c[i])
        bt = "{:^10s}".format("None") if i >= len(b) or b[i] == None else "{:^10.3f}".format(b[i])
        dt = "{:^10s}".format("None") if i >= len(d) or d[i] == None else "{:^10.3f}".format(d[i])

        print(str(i) + "\t" + xt + "\t" + yt + "\t" + ht + "\t" + at + "\t" + ct + "\t" + bt + "\t" + dt)

def print_ksi_eta(yValues, hValues, ksi, eta, cValues):
    print("i" + "\t\t" + "y" + "\t\t\t" + "h" + "\t\t\t" + "ksi" + "\t\t\t" + "eta")

    for i in range(0, len(ksi)):
        ht = "{:^10s}".format("None") if i >= len(hValues) or hValues[i] == None else "{:^10.5f}".format(hValues[i])
        yt = "{:^10s}".format("None") if i >= len(yValues) or yValues[i] == None else "{:^10.5f}".format(yValues[i])
        ksit = "{:^10s}".format("None") if i >= len(ksi) or ksi[i] == None else "{:^10.5f}".format(ksi[i])
        etat = "{:^10s}".format("None") if i >= len(eta) or eta[i] == None else "{:^10.5f}".format(eta[i])
        ct = "{:^10s}".format("None") if i >= len(cValues) or cValues[i] == None else "{:^10.5f}".format(cValues[i])
