...
def calcCValues(xValues, yValues, start, end):
    ...
    cValues = np.zeros(sizeX - 1)
    cValues[0] = start
    cValues[1] = end
    ksiValues = np.zeros(sizeX)
    tetaValues = np.zeros(sizeX)
    ksiValues[:2] = [start, end]
    tetaValues[:2] = [start, end]
...
def spline(x, xs, ys, range_):

    coefs = calculateCoefsSpline(xs, ys, *range_)
    index = finedIndex(xs, x)
    y = countPolynom(x, xs, index, coefs)
