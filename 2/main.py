...
def main():
    ...
    if table_type == "из таблицы":
        ...
        ys = st.number_input(
            "Введите y_start: ", -100.0, 100.0, 1.0, step=1.0, format="%.5f"
        )
        ye = st.number_input(
            "Введите y_end: ", -100.0, 100.0, 10.0, step=1.0, format="%.5f"
        )
        yst = int(st.number_input("Введите кол во узлов y: ", 1, 100, 10))

        zs = st.number_input(
            "Введите z_start: ", -100.0, 100.0, 1.0, step=1.0, format="%.5f"
        )
        ze = st.number_input(
            "Введите z_end: ", -100.0, 100.0, 10.0, step=1.0, format="%.5f"
        )
    ...
    y = st.number_input(
        "Введите аргумент y: ", -100.0, 100.0, 1.0, step=1.0, format="%.5f"
    )
    z = st.number_input(
        "Введите аргумент z: ", -100.0, 100.0, 1.0, step=1.0, format="%.5f"
    )
    ...
    if table_type == "Из таблицы":
        ...
        result = newton_3d_alt(
            (xs_, ys_, zs_), func, (nx + 1, ny + 1, nz + 1), (x, y, z)
        )
